...
//   const dictionary = ALPHABET.split('').reduce((acc, cur, index) => {
//     acc[cur] = ENCODE[index];
//
//     return acc;
//   }, {});
//
//   return str
//     .split('')
//     .map((ch) => dictionary[ch] || ch)
//     .join('');
// }

// оптимальное решение через методы строк
function encodeToRot13(str) {
...
//   'K♠',
// ];
// const cardMap = new Map(cardDictionary.map((el, i) => [el, i]));
//
...
const RANKS_PER_SUIT = RANKS.length; // 13

const cardMap = new Map(
  SUITS.flatMap((suit, si) =>
    RANKS.map((rank, ri) => [rank + suit, si * RANKS_PER_SUIT + ri])
  )
);
